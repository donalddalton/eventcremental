...
class EventRecorderSpec extends AnyFlatSpec with Matchers {
  ...
  "recorder" should "record an event" in {
    ...
    recorder.record(0) // record an event at t=0
  ...
  "recorder" should "record two events" in {
    val recorder = new EventRecorder("foo")
    recorder.record(0) // record an event at t=0
    recorder.record(1) // record an event at t=1
  ...
  "recorder" should "drop stale events when recording new events" in {
    ...
    recorder.record(1) // record an event at t=1
    recorder.getEvents shouldBe Seq(0, 1)
    recorder.record(3) shouldBe Some(0) // at t=3 the t=0 event becomes stale so it should get dropped
  ...
  "recorder" should "count events" in {
    ...
    recorder.record(1) // record an event at t=1
    recorder.getEvents shouldBe Seq(0, 1)
    recorder.getCount(1, 2) shouldBe 2 // set clock to t=1 and request last 2 ms of events (1, 0)
  }
  ...
  "recorder" should "filter events by timespan" in {
    ...
    recorder.record(0) // record an event at t=0
    recorder.record(1) // record an event at t=1
    recorder.record(3) // record an event at t=3
    recorder.record(4) // record an event at t=4
    recorder.record(5) // record an event at t=5
    recorder.getEvents shouldBe Seq(0, 1, 3, 4, 5)
    recorder.getCount(5, 2) shouldBe 3 // set clock to t=5 and request last 2 ms of events (5, 4, 3)
  }
  ...
  "recorder" should "not count stale events" in {
    ...
    recorder.record(1) // record an event at t=1
    recorder.getEvents shouldBe Seq(0, 1)
    recorder.getCount(4, 2) shouldBe 0 // set clock to t=4 and request last 2 ms of events
  }
